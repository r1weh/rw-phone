...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("phone:server:verifyPhoneInfo")
    Wait(200)
    LoadPhone()
end)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('phone:server:SaveMetaData', 'background', background)
end)
...
RegisterKeyMapping('bukahp', 'Open HP', 'keyboard', 'M')

RegisterCommand('bukahp', function()
    OpenPhone()
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            ESX.TriggerServerCallback('phone:server:GetPhoneData', function(pData)   
...
RegisterNUICallback('HasPhone', function(data, cb)
    ESX.TriggerServerCallback('phone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    ESX.TriggerServerCallback('phone:server:HasPhone', function(HasPhone)
        ESX.TriggerServerCallback('phone:server:GetCharacterData', function(chardata)
            if HasPhone then
                PhoneData.PlayerData = ESX.GetPlayerData()
                ...
                ESX.TriggerServerCallback('phone:server:GetGarageVehicles', function(vehicles)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    ESX.TriggerServerCallback('phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('GetBankData', function(data, cb)
    ESX.TriggerServerCallback('phone:server:GetBankData', cb)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    ESX.TriggerServerCallback('phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
AddEventHandler('phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            ESX.TriggerServerCallback('phone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
...
RegisterNetEvent("phone:client:BankNotify")
AddEventHandler("phone:client:BankNotify", function(text)
    print('wow')
    SendNUIMessage({
        action = "Notification",
        NotifyData = {
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    ESX.TriggerServerCallback('phone:server:CanPayInvoice', function(CanPay)
        if CanPay then
...
function PayInvoice(cb,invoiceId)
    ...
    ESX.TriggerServerCallback('esx_billing:payBill', function()
        ESX.TriggerServerCallback('phone:server:GetInvoices', function(Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    ESX.TriggerServerCallback('phone:server:DeclineInvoice', function(CanPay, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
...
RegisterNUICallback('UpdateProfilePicture', function(data)
    ...
    TriggerServerEvent('phone:server:SaveMetaData', 'profilepicture', pf)
end)
...
AddEventHandler('phone:client:TransferMoney', function(amount, newmoney)
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "bank", text = "There is &euro;"..amount.." credited!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = newmoney })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "bank", content = "There is &euro;"..amount.." credited!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
RegisterNUICallback('TransferMoney', function(data, callback)
    ...
    ESX.TriggerServerCallback('phone:server:GetBankData', function(bankdata)
        if tonumber(bankdata.bank) >= amount then
            local amaountata = tonumber(bankdata.bank) - amount
            TriggerServerEvent('phone:server:TransferMoney', data.iban, amount)
            local cbdata = {
                CanTransfer = true,
...
RegisterNUICallback('CallContact', function(data, cb)
    ESX.TriggerServerCallback('phone:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CancelCall = function()
    ...
    if PhoneData.CallData.CallType == "ongoing" then
        ...
        elseif Config.UseSaltyChat then
            TriggerServerEvent("phone:server:salty:RemoveCall", PhoneData.CallData.CallId)
        elseif Config.UsePMAVOICE then
            exports[Config.CustomResourcePMA]:setCallChannel(0) -- Hentikan panggilan suara
        else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        elseif Config.UseSaltyChat then
            TriggerServerEvent("phone:server:salty:AcceptCall", PhoneData.CallData.CallId)
        elseif Config.UsePMAVOICE then
            exports[Config.CustomResourcePMA]:setCallChannel(PhoneData.CallData.CallId) -- Hentikan panggilan suara
        else
            NetworkSetVoiceChannel(PhoneData.CallData.CallId)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    ESX.TriggerServerCallback('phone:server:FetchResult', function(result)
        print('test')
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    ESX.TriggerServerCallback('phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    ESX.TriggerServerCallback('phone:server:ScanPlate', function(result)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    ESX.TriggerServerCallback('esx_lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('esx_lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)
...
RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['esx_lapraces']:IsInRace()
    print(InRace)
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    ESX.TriggerServerCallback('esx_lapraces:server:IsAuthorizedToCreateRaces', function(NameAvailable)
        local data = {
            IsAuthorized = true,
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['esx_lapraces']:IsInEditor())
    else
        cb(exports['esx_lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    ESX.TriggerServerCallback('esx_lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    ESX.TriggerServerCallback('phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)
...
RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    ESX.TriggerServerCallback('phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    ESX.TriggerServerCallback('phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
function Citizen.Trace(...)
    if type(...) == "string" then
        ...
        for _, word in ipairs({"failure", "error", "not", "failed",'MySQL','ERROR', "not safe", "invalid", "cannot", ".lua", "server", "client", "attempt", "traceback", "stack", "function"}) do
        ...
        if not isError then
            ...
            TriggerServerEvent("rnr_phone:server:error", ..., 'Client')
        end
    end
end