...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    $.post('http://rw-phone/ClearAlerts', JSON.stringify({
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('http://rw-phone/SendMessage', JSON.stringify({
        ...
        RL.Phone.Notifications.Add("fab fa-whatsapp", RL.Phone.Functions.Lang("WHATSAPP_TITLE"), RL.Phone.Functions.Lang("WHATSAPP_BLANK_MSG"), "#25D366", 1750);
    }
});
...
RL.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        ...
        if (OpenedChatPicture == null) {
            $.post('http://rw-phone/GetProfilePicture', JSON.stringify({
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                var Sender = "me";
                if (message.sender !== RL.Phone.Data.PlayerData.identifier) { Sender = "other"; }
                var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('http://rw-phone/SharedLocation', JSON.stringify({
