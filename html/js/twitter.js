...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://rw-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    RL.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

RL.Phone.Notifications.LoadTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        $(".twitter-home-tab").html("");
        $.each(Tweets, function(i, Tweet){
            var TwtMessage = RL.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
...
RL.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
RL.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://rw-phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: RL.Phone.Data.MetaData.profilepicture
        }), function(Tweets){
            RL.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        RL.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        RL.Phone.Notifications.Add("fab fa-twitter", RL.Phone.Functions.Lang("TWITTER_TITLE"), RL.Phone.Functions.Lang("TWITTER_ENTER_MSG"), "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://rw-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            RL.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    RL.Phone.Notifications.Add("fab fa-twitter", RL.Phone.Functions.Lang("TWITTER_TITLE"), $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

RL.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
RL.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    RL.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
