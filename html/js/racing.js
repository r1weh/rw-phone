...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(RL.Phone.Data.PlayerData.identifier, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_ALREADY_STARTED"), "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(RL.Phone.Data.PlayerData.identifier, race.RaceData.Racers);
            var Creator = IsCreator(RL.Phone.Data.PlayerData.identifier, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('http://rw-phone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('http://rw-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('http://rw-phone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('http://rw-phone/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            ...
                            RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_INEDITOR"), "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_ALREADY_INRACE"), "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('http://rw-phone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('http://rw-phone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('http://rw-phone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
        ...
        $(".racing-setup-information-distance").html('Afstand: '+TrackData.Distance+' m');
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('http://rw-phone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('http://rw-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('http://rw-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_INRACE"), "#1DA1F2");
                    }
                });
            } else {
                RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_ALREADY_CREATED"), "#1DA1F2");
            }
        } else {
            RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_CANTSTART"), "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        $.post('http://rw-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('http://rw-phone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_CANTTHISNAME"), "#1DA1F2");
                }
            } else {
                RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_CANTSTART"), "#1DA1F2");
            }
        ...
        RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_ENTER_TRACK"), "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('http://rw-phone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('http://rw-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('http://rw-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('http://rw-phone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Selecteer een Track');
                                        $(".racing-setup-information-creator").html('Selecteer een Track');
                                        $(".racing-setup-information-wr").html('Selecteer een Track');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Selecteer een Track");
                                    });
                                } else {
                                    RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_CANT_THIS_TIME"), "#1DA1F2");
                                }
                            });
                        } else {
                            RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_ENTER_ROUNDS"), "#1DA1F2");
                        }
                    } else {
                        RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_CHOSEN_TRACK"), "#1DA1F2");
                    }
                }
            })
        } else {
            RL.Phone.Notifications.Add("fas fa-flag-checkered", RL.Phone.Functions.Lang("RACING_TITLE"), RL.Phone.Functions.Lang("RACING_ALREADY_ACTIVE"), "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('http://rw-phone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
