...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://rw-phone/ClearRecentAlerts');
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== RL.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            $.post('http://rw-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        RL.Phone.Notifications.Add("fa fa-phone-alt", RL.Phone.Functions.Lang("PHONE_TITLE"), RL.Phone.Functions.Lang("PHONE_MSG_YOURSELF"), "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://rw-phone/EditContact', JSON.stringify({
        ...
        RL.Phone.Notifications.Add("fas fa-exclamation-circle", RL.Phone.Functions.Lang("CONTACTS_EDIT_TITLE"), RL.Phone.Functions.Lang("ALLFIELDS"));
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://rw-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://rw-phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://rw-phone/RemoveSuggestion', JSON.stringify({
        ...
        RL.Phone.Notifications.Add("fas fa-exclamation-circle", RL.Phone.Functions.Lang("CONTACTS_ADD_TITLE"), RL.Phone.Functions.Lang("ALLFIELDS"));
    }
});
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://rw-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://rw-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://rw-phone/CancelOngoingCall');
});
