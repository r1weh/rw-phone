...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (RL.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('http://rw-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    RL.Phone.Data.PlayerData.money.bank = parseInt(RL.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    RL.Phone.Notifications.Add("fas fa-university", "bank", "Er is &euro; "+Price+",- afgeschreven!", "#badc58", 2500);
                } else {
                    RL.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg geld..", "#badc58", 1500);
                }
            });
        } else {
            RL.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg geld..", "#badc58", 1500);
        ...
        RL.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Vul alles in!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('http://rw-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    RL.Phone.Data.PlayerData.money.bank = parseInt(RL.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    RL.Phone.Notifications.Add("fas fa-university", "bank", "Er is &euro; "+Price+",- bijgeschreven!", "#badc58", 2500);
                } else {
                    RL.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg Qbits..", "#badc58", 1500);
                }
            });
        } else {
            RL.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg Qbits..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://rw-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        RL.Phone.Notifications.Add("fas fa-university", "Crypto", "Dit Wallet-ID bestaat niet!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        RL.Phone.Notifications.Add("fas fa-university", "Crypto", "Je hebt "+Coins+",- overgemaakt naar "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                RL.Phone.Notifications.Add("fas fa-university", "Crypto", "Je kan niet naar jezelf overmaken..", "#badc58", 2500);
            }
        } else {
            RL.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg Qbits..", "#badc58", 1500);
        }
