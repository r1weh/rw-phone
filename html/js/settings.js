...
$(document).on('click', '#accept-background', function(e){
    ...
    if (hasCustomBackground === false) {
        RL.Phone.Notifications.Add("fas fa-paint-brush", RL.Phone.Functions.Lang("SETTINGS_TITLE"), RL.Phone.Settings.Backgrounds[RL.Phone.Settings.Background].label+" is ingesteld!")
        ...
        RL.Phone.Notifications.Add("fas fa-paint-brush", RL.Phone.Functions.Lang("SETTINGS_TITLE"), RL.Phone.Functions.Lang("BACKGROUND_SET"))
        RL.Phone.Animations.TopSlideUp(".settings-"+RL.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $(".phone-background").css({"background-image":"url('"+RL.Phone.Settings.Background+"')"});
    }

    $.post('http://rw-phone/SetBackground', JSON.stringify({
...
$(document).on('click', '#accept-profilepicture', function(e){
    ...
    if (ProfilePicture === "default") {
        RL.Phone.Notifications.Add("fas fa-paint-brush", RL.Phone.Functions.Lang("SETTINGS_TITLE"), RL.Phone.Functions.Lang("POFILE_DEFAULT"))
        RL.Phone.Animations.TopSlideUp(".settings-"+RL.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="./img/default.png">');
    } else {
        RL.Phone.Notifications.Add("fas fa-paint-brush", RL.Phone.Functions.Lang("SETTINGS_TITLE"), RL.Phone.Functions.Lang("PROFILE_SET"))
        RL.Phone.Animations.TopSlideUp(".settings-"+RL.Phone.Settings.OpenedTab+"-tab", 200, -100);
        console.log(ProfilePicture)
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="'+ProfilePicture+'">');
    }
    $.post('http://rw-phone/UpdateProfilePicture', JSON.stringify({
