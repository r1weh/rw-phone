...
$(document).on('click', '.bank-app-account', function(e){
    ...
    RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), "Bank account No. copied!", "#badc58", 1750);
});
...
RL.Phone.Functions.DoBankOpen = function(data) {
    $(".bank-app-account-number").val("IBAN: " + data.iban);
    $(".bank-app-account-balance").html("&euro; "+data.bank.toFixed());
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        if (amountData >= amount) {
            $.post('http://rw-phone/TransferMoney', JSON.stringify({
                iban: iban,
                amount: amount
            }), function(data){
                if (data.CanTransfer) {
                    ...
                    RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), "You have &euro; "+amount+",- transferred!", "#badc58", 1500);
                } else {
                    RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), RL.Phone.Functions.Lang("BANK_DONT_ENOUGH"), "#badc58", 1500);
                }
            })
            RL.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
        } else {
        ...
        RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), RL.Phone.Functions.Lang("ALLFIELDS"), "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://rw-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), "You have &euro;"+InvoiceData.amount+" paid!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://rw-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
    ...
    RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), "Je hebt &euro;"+InvoiceData.amount+" betaald!", "#badc58", 1500);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        RL.Phone.Notifications.Add("fas fa-university", RL.Phone.Functions.Lang("BANK_TITLE"), RL.Phone.Functions.Lang("BANK_NOIBAN"), "#badc58", 2500);
    }
    RL.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});